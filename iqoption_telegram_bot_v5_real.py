#!/usr/bin/env python3
"""
IQ Option Telegram Bot v5 - Real Connection Version
- Connects to real IQ Option (Demo by default)
- Shows real balance on Dashboard
- Generates signals from real candle data
...
# ==================== GLOBAL STATE ====================
user_data = {}
user_iq = {}  # Stores IQOption connection per user
...
def generate_high_probability_signal(df):
    ...
    # Very strict conditions for high probability
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "email":
        ...
        await update.message.reply_text("🔄 Connecting to IQ Option... Please wait.")

        # Connect to real IQ Option (Demo by default)
        ...
        if iq.connected:
            ...
            await update.message.reply_text("❌ Login failed. Please check your email/password and try again.")
...
async def show_real_signals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not signals_text:
        text = "No high-probability signals right now. Try again in a few minutes."
    else:
...
def main():
    ...
    print("✅ IQ Option Bot v5 (Real Connection) is running...")
    app.run_polling()
