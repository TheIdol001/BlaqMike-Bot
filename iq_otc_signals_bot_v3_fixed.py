#!/usr/bin/env python3
"""
IQ Option OTC Signals Bot v3 (Fixed - Lightweight + Proper JobQueue)
- All indicators calculated with pure pandas (no numba / pandas_ta)
- High-probability strategy (65%+)
- Auto signals every 3 minutes to channel (with proper JobQueue init)
- Martingale up to 3 steps + Win-rate tracking
...
def format_signal(sig, user_id):
    ...
    return (
        f"🎯 **HIGH-PROB OTC SIGNAL** (v3 - Fixed)\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🚀 **IQ Option OTC Signals Bot v3** (Fixed & Lightweight)\n\n"
...
def main():
    ...
    # Proper JobQueue initialization (fixes 'NoneType' error)
    job_queue = JobQueue()
    ...
    # Start auto signals only if JobQueue is available
    if app.job_queue:
        ...
        print("⚠️ JobQueue not available - auto signals disabled")

    print("✅ OTC Signals Bot v3 (Fixed) is running...")
    app.run_polling()
